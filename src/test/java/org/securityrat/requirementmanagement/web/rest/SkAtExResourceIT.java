...
/**
 * Integration tests for the {@link SkAtExResource} REST controller.
 */
@SpringBootTest(classes = {RequirementManagementApp.class, TestSecurityConfiguration.class})
public class SkAtExResourceIT {
     ...
     * if they test an entity which requires the current entity.
     */
    public static SkAtEx createUpdatedEntity(EntityManager em) {
        SkAtEx skAtEx = new SkAtEx();
        return skAtEx;
    }
    ...
    private void defaultSkAtExShouldBeFound(String filter) throws Exception {
        ...
        restSkAtExMockMvc.perform(get("/api/sk-at-exes/count?sort=id,desc&" + filter))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
    ...
    public void updateSkAtEx() throws Exception {
        ...
        // Update the skAtEx
        SkAtEx updatedSkAtEx = skAtExRepository.findById(skAtEx.getId()).get();
